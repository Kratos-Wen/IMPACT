...
def build_full_node_set(reference_json: dict,
                        alias_map: Dict[str, str] | None,
                        include_event_types: Set[str]) -> Set[str]:
    nodes = set()
    for component in reference_json.get("components", []):
        raw_name = component.get("name")
        if not raw_name:
            continue
        name = alias_map.get(raw_name, raw_name) if alias_map else raw_name
        for event_type in include_event_types:
            nodes.add(f"{name}__{event_type}")
    return nodes


def resolve_annotation_paths(asr_dir: str | None,
                             split_dir: str | None,
                             bundle_split: str,
                             impact_split: str,
                             ...
                             camera: str) -> List[str]:
    if split_dir:
        gt_root = annotation_dir or default_impact_annotation_dir("../data")
        paths = []
        missing = []
        for entry in read_bundle_entries(split_dir, bundle_split, impact_split):
            video_stem = entry_to_camera_stem(entry, camera)
            gt_name = annotation_filename_from_video_stem(video_stem)
        ...
        if missing:
            ...
            raise FileNotFoundError(
                f"Missing {len(missing)} annotation files for bundle {bundle_split}.{impact_split}. "
                f"First missing paths:\n{preview}"
            )
        return sorted(paths)

    if not asr_dir:
        raise ValueError("Either --asr_dir or --split_dir must be provided.")

    paths = sorted(glob.glob(os.path.join(asr_dir, "*.json")))
    if not paths:
        raise FileNotFoundError(f"No ASR JSON files found under: {asr_dir}")
    return paths


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--asr_dir", type=str, default=None, help="Folder containing ASR JSON files (*.json).")
    ...
    ap.add_argument("--split_dir", type=str, default=None,
                    help="Optional bundle split directory, e.g. ../data/IMPACT/splits_ASR_front_only_v1")
    ap.add_argument("--impact_split", type=str, default="split1",
                    help="Bundle suffix when --split_dir is used, e.g. split1")
    ap.add_argument("--bundle_split", type=str, default="train", choices=["train", "val", "test"],
                    help="Which bundle split to use when --split_dir is used")
    ap.add_argument("--annotation_dir", type=str, default=None,
                    help="Annotation directory. Defaults to ../data/IMPACT_ASR when --split_dir is used")
    ap.add_argument("--camera", type=str, default="front", choices=["front", "left", "right", "top", "ego"],
                    help="Camera/view to resolve when --split_dir is used")
    ...
    paths = resolve_annotation_paths(
        args.asr_dir,
        args.split_dir,
        args.bundle_split,
        args.impact_split,
    ...
    reference_json = load_json(paths[0])
    nodes |= build_full_node_set(reference_json, alias_map, include_types)
    ...
    graph = {
        "meta": {
            "num_videos": n_videos,
            "source": "bundle_split" if args.split_dir else "directory_glob",
            "bundle_split": args.bundle_split if args.split_dir else None,
            "impact_split": args.impact_split if args.split_dir else None,
            "camera": args.camera if args.split_dir else None,
