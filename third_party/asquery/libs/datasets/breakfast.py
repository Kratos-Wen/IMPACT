...
@register_dataset("ego4exo_front")
@register_dataset("ego4exo_left")
@register_dataset("ego4exo_right")
@register_dataset("ego4exo_top")
@register_dataset("Ego4Exo_front")
@register_dataset("Ego4Exo_left")
@register_dataset("Ego4Exo_right")
@register_dataset("Ego4Exo_top")
@register_dataset("impact")
@register_dataset("IMPACT")
class BreakfastDataset(Dataset):
    def __init__(
        ...
        self.skip_missing_features = skip_missing_features
    ...
    def _load_annotaion(self, anno_file, gt_dir, label_dict):
        ...
        for anno_file_name in anno_file_names:
            anno_file_name = anno_file_name.strip()
            if not anno_file_name:
                continue
            if not anno_file_name.endswith('.txt'):
                anno_file_name = anno_file_name + '.txt'
            video_id = os.path.splitext(anno_file_name)[0]
            feat_file = self._resolve_feature_file(video_id)
            if feat_file is None:
                num_missing_feats += 1
                if self.skip_missing_features:
                    continue
                raise FileNotFoundError(
                    f"Feature file not found for video_id={video_id} in {self.feat_folder}"
                )

            anno_file_path = os.path.join(gt_dir, anno_file_name)
        ...
        if num_missing_feats > 0:
            print(f"[Dataset] Missing features skipped: {num_missing_feats}")
        return dict_db

    def _resolve_feature_file(self, video_id):
        candidates = [video_id]
        ...
        for stem in candidates:
            if stem in seen:
                continue
            seen.add(stem)
            filename = os.path.join(self.feat_folder, self.file_prefix + stem + self.file_ext)
    ...
    def _get_seg_label(self, anno_cont, label_dict):
        ...
        for idx in range(1, len(anno_cont)):
            if anno_cont[idx] != anno_cont[idx-1]:
                f_s.append(idx)
        if len(f_s) == 0:
            segments.append([0, len(anno_cont) - 1])
            labels.append(label_dict[anno_cont[0].strip()])
        else:
    ...
    def __getitem__(self, idx):
        ...
        # load features
        filename = video_item.get('feat_file')
        if filename is None:
            filename = self._resolve_feature_file(video_item['id'])
            if filename is None:
                raise FileNotFoundError(f"Feature file not found for video_id={video_item['id']}")
        feats = np.load(filename).astype(np.float32)

        # Normalize feature layout to channel-first C x T.
        if feats.ndim != 2:
            raise ValueError(f"Expected 2D feature array, got shape {feats.shape} for {filename}")
        if feats.shape[0] != self.input_dim and feats.shape[1] == self.input_dim:
            feats = feats.T
