...
def compute_phase_metrics_from_arrays(gt_arr, pred_arr, class_names):
    phase_ids = infer_phase_class_ids(class_names)
    if phase_ids is None:
        return {}

    gt_arr = np.asarray(gt_arr).reshape(-1)
    ...
    metrics["Phase_Acc"] = float((gt_arr == pred_arr).mean() * 100.0)
    metrics["Phase_MacroF1"] = float(np.mean(list(per_class_f1.values())) * 100.0)
    metrics["Phase_F1_Normal"] = float(per_class_f1["normal"] * 100.0)
    metrics["Phase_F1_Anomaly"] = float(per_class_f1["anomaly"] * 100.0)
    metrics["Phase_F1_Recovery"] = float(per_class_f1["recovery"] * 100.0)
    return metrics
...
class SegEval(object):
    ...
    def evaluate(self, results):
        ...
        phase_gt = np.concatenate([gt.detach().cpu().numpy().reshape(-1) for gt in gts])
        phase_pred = np.concatenate([pred.detach().cpu().numpy().reshape(-1) for pred in preds])
        eval_results.update(compute_phase_metrics_from_arrays(phase_gt, phase_pred, self.class_names))
