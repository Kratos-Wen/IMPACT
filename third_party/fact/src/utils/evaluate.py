...
def infer_phase_class_ids(index2label):
    if not index2label:
        return None
    name_to_idx = {_normalize_label_name(name): idx for idx, name in index2label.items()}
...
def compute_phase_metrics(gt_list, pred_list, index2label):
    phase_ids = infer_phase_class_ids(index2label)
...
class Checkpoint():
    def __init__(self, iteration, bg_class=[], eval_edit=True, index2label=None):
        self.iteration = iteration
        self.videos = {}

        self.bg_class = bg_class
        self.eval_edit = eval_edit # turn off this can save computation time
        self.index2label = index2label or {}
    ...
    def compute_metrics(self):
        ...
        self.metrics.update(compute_phase_metrics(gt_list, pred_list, self.index2label))
