...
class FACT(nn.Module):
    ...
    def __init__(self, cfg, in_dim, n_classes):
        ...
        self.is_atr_task = str(cfg.impact_task if 'impact_task' in cfg else '').upper().startswith('ATR_')
    ...
    def _loss_one_video_atr(self, label_info):
        target = label_info["target"].float()
        valid_mask = label_info["mask"].float().unsqueeze(-1)
        denom = torch.clamp(valid_mask.sum() * self.num_classes, min=1.0)

        loss_list = []
        for block in self.block_list:
            frame_logits = block.frame_clogit.squeeze(1)
            frame_loss = F.binary_cross_entropy_with_logits(
                frame_logits,
                target,
                reduction='none',
            )
            frame_loss = (frame_loss * valid_mask).sum() / denom
            loss_list.append(frame_loss)
    ...
    def forward(self, seq_list, label_list, compute_loss=False):
        ...
        for i, (seq, label) in enumerate(zip(seq_list, label_list)):
            seq = seq.unsqueeze(1)
            if self.is_atr_task:
                self._forward_one_video(seq, None)
                prob = self.block_list[-1].eval_atr()
                save_data = {'prob': utils.to_numpy(prob)}
                save_list.append(save_data)

                if compute_loss:
                    loss = self._loss_one_video_atr(label)
...
class Block(nn.Module):
    ...
    def eval_atr(self):
        return torch.sigmoid(self.frame_clogit.squeeze(1))
