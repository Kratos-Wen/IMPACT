...
from ltc.utils.metrics import calculate_metrics, infer_phase_class_ids, load_label_names
...
def _resolve_mapping_path(cfg: CfgNode) -> str:
    mapping_file = cfg.DATA.MAPPING_FILE
    if os.path.isabs(mapping_file):
        return mapping_file
    return join(cfg.DATA.PATH_TO_DATA_DIR, mapping_file)
...
class TrainMeter(object):
    ...
    def __init__(self,
        ...
        self._label_names = load_label_names(_resolve_mapping_path(cfg))
        self._is_atr_task = str(cfg.MODEL.LOSS_FUNC).lower() == "masked_bce"
        self._is_phase_task = infer_phase_class_ids(self._label_names) is not None

        self.metrics_dict = {}
        if not self._is_atr_task:
            self.metrics_dict = {
                "MoF": [],
                "Edit": [],
                "F1@10": [],
    ...
    def update_stats(self,
        ...
        :return:
        """
        mb_size = 1 if target is None else target.shape[0]
        video_metrics = {}
        if not self._is_atr_task and target is not None and prediction is not None:
            target = target.detach().cpu()
            prediction = prediction.detach().cpu()
            video_metrics = calculate_metrics(target, prediction, self._ignore_idx, self._label_names)
        ...
        for name, loss in (loss_dict or {}).items():
            loss = loss.item()
...
class ValMeter(object):
    ...
    def log_epoch_stats(self, cur_epoch: int):
        ...
        for name, metric_list in self.metrics_dict.items():
            metric_mean = np.mean(metric_list)
            metric_means[name] = metric_mean
            stats[name] = metric_mean
            if self.writer:
                self.writer.add_scalar(f"1-Val_Metric/{name}", metric_mean, global_step=cur_epoch)
        if "Phase_MacroF1" in metric_means:
            metrics_sum = (
                metric_means["Phase_MacroF1"] +
                metric_means.get("Phase_Acc", 0.0) +
                metric_means.get("Phase_F1_Anomaly", 0.0) +
                metric_means.get("Phase_F1_Recovery", 0.0)
            )
        else:
            for name, metric_mean in metric_means.items():
                if name == 'Edit':
