...
def infer_phase_class_names(class_names):
    if class_names is None:
        return None
    normalized = {_normalize_label_name(name) for name in class_names}
    required = {'normal', 'anomaly', 'recovery'}
    if not required.issubset(normalized):
        return None
    return ['normal', 'anomaly', 'recovery']


def compute_phase_metrics(gt_labels, pred_labels, class_names):
    phase_names = infer_phase_class_names(class_names)
    if phase_names is None:
        return {}

    gt_arr = np.asarray([_normalize_label_name(label) for label in gt_labels])
    pred_arr = np.asarray([_normalize_label_name(label) for label in pred_labels])
    eps = 1e-8
    per_class_f1 = {}
    for phase_name in phase_names:
        tp = np.sum((gt_arr == phase_name) & (pred_arr == phase_name))
        fp = np.sum((gt_arr != phase_name) & (pred_arr == phase_name))
        fn = np.sum((gt_arr == phase_name) & (pred_arr != phase_name))
        precision = tp / (tp + fp + eps)
        recall = tp / (tp + fn + eps)
        per_class_f1[phase_name] = 2.0 * precision * recall / (precision + recall + eps)

    return {
        'Phase_Acc': float((gt_arr == pred_arr).mean() * 100.0),
        'Phase_MacroF1': float(np.mean(list(per_class_f1.values())) * 100.0),
        'Phase_F1_Normal': float(per_class_f1['normal'] * 100.0),
        'Phase_F1_Anomaly': float(per_class_f1['anomaly'] * 100.0),
        'Phase_F1_Recovery': float(per_class_f1['recovery'] * 100.0),
    }
...
def func_eval(label_dir, pred_dir, video_list, class_names=None):
    ...
    for vid in video_list:
        ...
        if len(gt_content) != len(pred_content):
            raise ValueError(
                f'Prediction/GT length mismatch for {vid}: '
                f'gt={len(gt_content)} pred={len(pred_content)} '
                f'(gt_file={gt_file}, pred_file={pred_file})'
            )
        phase_gt_all.extend(gt_content)
        phase_pred_all.extend(pred_content)
    ...
    phase_metrics = compute_phase_metrics(phase_gt_all, phase_pred_all, class_names)
    return acc, edit, f1s, phase_metrics
