...
def is_impact_eval(args):
    joined = " ".join([
        str(args.run_path),
        str(args.rec_path),
        str(args.psr_label_path),
        str(args.procedure_info),
    ]).lower()
    return 'impact' in joined


def infer_num_dig_psr(procedure_info_path: str, default: int) -> int:
    if procedure_info_path is None:
        return default
    try:
        proc_info = ut.get_procedure_info(procedure_info_path)
    except Exception:
        return default
    if not proc_info:
        return default
    return max(int(step["state_idx"]) for step in proc_info) + 1
...
def quick_setting(args):
    ...
    elif is_impact_eval(args):
        args.FPS = 10
        args.width = 1280
        args.height = 720
        args.num_dig_psr = infer_num_dig_psr(args.procedure_info, args.num_dig_psr)
    
    else:
        raise NotImplementedError(f"Only implement for IndustReal, MECCANO, and IMPACT.")
...
if __name__ == '__main__':
    ...
    impact_flag = is_impact_eval(args)
    if 'industreal' in str(args.psr_label_path).lower():
        ...
        print("Evaluate on IMPACT dataset...")
        categories = ['background'] + [f'state_{idx}' for idx in range(args.num_dig_psr)]
        assy_Flag = None

    else:
        raise NotImplementedError(f"Currently only support meccano, industreal, and IMPACT.")
    ...
    for impl in implementations:
        ...
        for i, rec in enumerate(recordings):
            ...
            elif psr_utils.video_contains_errors(gt, psr_config["proc_info"], Path(args.psr_label_path) / args.split / rec.name, assy_only= assy_Flag):
