...
def train_epoch(
    ...
    for cur_iter, (batch_dict) in train_bar:
        ...
        if is_atr_task:
            loss_dict = loss_func(logits, batch_dict['targets'], batch_dict['atr_mask'])
        else:
        ...
        prediction = None if is_atr_task else misc.prepare_prediction(logits)

        # Update and log stats.
        train_meter.update_stats(
            target=None if is_atr_task else batch_dict['targets'],
...
def eval_epoch(
    ...
    for cur_iter, (batch_dict) in tqdm(enumerate(val_loader), total=len(val_loader)):
        ...
        if is_atr_task:
            ...
            val_meter.update_stats(loss_dict=loss_dict)
        else:
            loss_dict = loss_func(logits, batch_dict['targets'])
    ...
    # Log epoch stats.
    if is_atr_task:
        mapping_path = cfg.DATA.MAPPING_FILE if os.path.isabs(cfg.DATA.MAPPING_FILE) else os.path.join(
            cfg.DATA.PATH_TO_DATA_DIR,
            cfg.DATA.MAPPING_FILE,
        )
        segments_dir = cfg.DATA.ATR_SEGMENTS_DIR if os.path.isabs(cfg.DATA.ATR_SEGMENTS_DIR) else os.path.join(
            cfg.DATA.PATH_TO_DATA_DIR,
            cfg.DATA.ATR_SEGMENTS_DIR,
        )
        val_metrics = evaluate_atr_predictions(
            atr_predictions,
            segments_dir,
        ...
        avg_loss = {
            name: np.round(loss / max(val_meter.num_samples, 1), decimals=4)
            for name, loss in val_meter.loss_sums.items()
        }
        stats = {
            "_type": "val_epoch",
            "epoch": "{}/{}".format(cur_epoch + 1, cfg.SOLVER.MAX_EPOCH),
            "avg_loss": avg_loss,
            "mem": int(np.ceil(misc.gpu_mem_usage())),
        }
        stats.update(avg_loss)
        stats.update(val_metrics)
        val_metrics["metrics_sum"] = val_metrics.get("ATR_mAP_present", 0.0)
