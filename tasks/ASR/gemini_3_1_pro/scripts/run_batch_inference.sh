...
OUTPUT_ROOT="${5:-$ROOT_DIR/outputs/asr/gemini_3_1_pro/${RUN_TAG}/predictions}"
LOG_ROOT="${6:-$ROOT_DIR/logs/asr/gemini_3_1_pro/${RUN_TAG}}"
...
"${PYTHON_BIN}" "${ROOT_DIR}/tasks/ASR/gemini_3_1_pro/predict_states_and_steps.py" \
  --video_dir "${VIDEO_DIR}" \
  --asr_dir "${ASR_JSON_DIR}" \
  --procedure_graph "${ROOT_DIR}/tasks/ASR/gemini_3_1_pro/configs/procedure_graph.json" \
  --component_alias "${ROOT_DIR}/tasks/ASR/gemini_3_1_pro/configs/component_alias.json" \
  --out_dir "${OUTPUT_ROOT}" \
  --model "${MODEL_NAME}" \
