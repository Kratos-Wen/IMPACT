# Gemini 3.1 Pro for ASR

This directory provides the released IMPACT `ASR` wrapper for `Gemini 3.1 Pro`.

## Path Convention

- annotation assets: `dataset/ASR/annotations`
- split assets: `dataset/ASR/splits_front_only_v1`
- method configs: `tasks/ASR/gemini_3_1_pro/configs`
- default outputs: `outputs/asr/gemini_3_1_pro/<run_tag>/predictions`
- default logs: `logs/asr/gemini_3_1_pro/<run_tag>`
- raw videos: external
...
- `SPLIT_ID`: released split id, currently `split1`
- `CAMERA`: released public protocol uses `front`

## Scripts

- `scripts/run_batch_inference.sh`: runs the released batch prompting pipeline and writes prediction JSON files
- `scripts/eval_predictions.sh`: evaluates saved prediction JSON files as ASR

## Examples

```bash
GEMINI_API_KEY=... \
bash tasks/ASR/gemini_3_1_pro/scripts/run_batch_inference.sh /path/to/videos
```

```bash
bash tasks/ASR/gemini_3_1_pro/scripts/eval_predictions.sh \
  outputs/asr/gemini_3_1_pro/<run_tag>/predictions test split1 front
```

## Notes

- The released prompting pipeline jointly predicts `states_over_time` and completed step lists; the ASR evaluator uses the state trajectory only.
